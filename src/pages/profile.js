...
function profile(){
	...
	useEffect(() =>{
		...
		const getNewValue = localStorage.getItem('Values');
		const splitArray = getNewValue.split('@');
		const firstIndex = splitArray[0];
		setGetWord(firstIndex);
	...
	if(isLoggedIn !== null){
		...
		<div className=" bg-emerald-900 w-full h-screen py-4">
			<h3 className="text md:text-[60px] pt-2 md:pt-10 text-3xl text-emerald-200 font-bold text-center">Welcome {getWord}</h3>
			<div className="bg-emerald-100 w-9/12 mt-20 mx-auto rounded-lg h-[500px]">
				...
				<div className="pt-5 px-5 ">
					...
					<div className=" text-center text-[13px] md:text-xl pt-7">					
						<p className="pb-3 text-emerald-700 font-semibold tracking-wide">Name: {getWord}</p>
				...
				<button onClick={moveToLogin} className="flex hover:bg-emerald-400 hover:text-emerald-100 transition ease-in-out duration-500 mx-auto mt-10 md:mt-20 bg-emerald-300 text-2xl text-emerald-900 py-1 rounded-lg font-bold px-12">Logout</button>
			</div>
			<ToastContainer />
		</div>
		)
